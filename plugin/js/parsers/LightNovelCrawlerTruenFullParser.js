...
parserFactory.register("truyentr.info", () => new LightNovelCrawlerTruenFullParser());


class LightNovelCrawlerTruenFullParser extends Parser {
    constructor() {
        super();
    }
    ...
    findContent(dom) {
        const selector = "#chapter-c, .chapter-c";
        if (selector) {
    ...
    extractTitleImpl(dom) {
        const selector = "h3.title, h1.title";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = ".info a[itemprop=\"author\"]";
        if (selector) {
