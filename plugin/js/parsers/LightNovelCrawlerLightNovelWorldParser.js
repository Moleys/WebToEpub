...
parserFactory.register("lightnovel.world", () => new LightNovelCrawlerLightNovelWorldParser());


class LightNovelCrawlerLightNovelWorldParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "div#chapter_content ul li a";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "div#content_detail";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = ".book_info_l img";
        if (selector) {
