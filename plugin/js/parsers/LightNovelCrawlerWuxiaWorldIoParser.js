...
parserFactory.register("wuxiaworld.name", () => new LightNovelCrawlerWuxiaWorldIoParser());


class LightNovelCrawlerWuxiaWorldIoParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "#list_chapter .chapter-list a";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "div.content-area";
        if (selector) {
    ...
    extractTitleImpl(dom) {
        const selector = "div.entry-header h1";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = "div.truyen_info_right li";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = "span.info_image img";
        if (selector) {
