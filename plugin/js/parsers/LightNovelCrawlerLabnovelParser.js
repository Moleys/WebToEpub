...
parserFactory.register("labnovel.ru", () => new LightNovelCrawlerLabnovelParser());


class LightNovelCrawlerLabnovelParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "div.chapter-list a";
        if (!selector) {
    ...
    extractTitleImpl(dom) {
        const selector = ".main-block-book__title h1";
    ...
    findCoverImageUrl(dom) {
        const selector = ".main-block-book__img img";
        if (selector) {
    ...
    getInformationEpubItemChildNodes(dom) {
        const selector = "div[class*='_desc']";
