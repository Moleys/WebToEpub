...
parserFactory.register("phoenixnovels.com.br", () => new LightNovelCrawlerPhoenixNovelsParser());


class LightNovelCrawlerPhoenixNovelsParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "li.parent.has-child";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = ".reading-content";
        if (selector) {
    ...
    removeUnwantedElementsFromContentElement(element) {
        const removeSelectors = ["div#padSection", "div.padSection"];
        const removeTags = [];
