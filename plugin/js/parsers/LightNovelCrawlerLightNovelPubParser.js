...
parserFactory.register("lightnovelpub.org", () => new LightNovelCrawlerLightNovelPubParser());


class LightNovelCrawlerLightNovelPubParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "div.novel-info div.genre-tags span";
        if (!selector) {
    ...
    extractTitleImpl(dom) {
        const selector = "div.novel-cover-container img[src]";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = "p.novel-author";
        if (selector) {
