...
parserFactory.register("noveltranslate.com", () => new LightNovelCrawlerNovelTranslateParser());


class LightNovelCrawlerNovelTranslateParser extends Parser {
    constructor() {
        super();
    }
    ...
    findContent(dom) {
        const selector = "div.reading-content p";
        if (selector) {
            let nodes = [...dom.querySelectorAll(selector)];
            if (nodes.length > 0) {
                if (nodes.length === 1) {
