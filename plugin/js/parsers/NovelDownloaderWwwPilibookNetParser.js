...
parserFactory.register("www.pilibook.net", () => new NovelDownloaderWwwPilibookNetParser());

class NovelDownloaderWwwPilibookNetParser extends Parser {
    constructor() {
        super();
    ...
    async getChapterUrls(dom) {
        let chapterListLink = dom.querySelector(PILIBOOK_CONFIG.selectors.CHAPTER_LIST_LINK);
        if (!chapterListLink) {
            return [];
        }
        let tocDom = (await HttpClient.wrapFetch(chapterListLink.href)).responseXML;
        let links = [...tocDom.querySelectorAll(PILIBOOK_CONFIG.selectors.CHAPTER_LINKS)];
    ...
    extractTitleImpl(dom) {
        return dom.querySelector(PILIBOOK_CONFIG.selectors.BOOK_TITLE);
    }

    extractAuthor(dom) {
        let authorLabel = dom.querySelector(PILIBOOK_CONFIG.selectors.BOOK_AUTHOR);
    ...
    extractDescription(dom) {
        let intro = dom.querySelector(PILIBOOK_CONFIG.selectors.BOOK_INTRO);
        return intro?.textContent?.trim() ?? super.extractDescription(dom);
    }

    findCoverImageUrl(dom) {
        let img = dom.querySelector(PILIBOOK_CONFIG.selectors.BOOK_COVER);
...
function extractChapterIdFromUrl(url) {
    let match = url.match(PILIBOOK_CONFIG.urlPatterns.CHAPTER_ID_REGEX);
    return match ? match[1] : null;
}

async function fetchWeimengChapter(chapterId, chapterUrl) {
    let params = new URLSearchParams({
        action: PILIBOOK_CONFIG.api.CHAPTER_ACTION,
        cid: chapterId,
        format: "1"
    });
    let origin = chapterUrl ? new URL(chapterUrl).origin : document.location.origin;
    let apiUrl = `${origin}${PILIBOOK_CONFIG.api.ENDPOINT_PATTERN}?${params.toString()}`;
    ...
    if (!data || data.code !== PILIBOOK_CONFIG.api.SUCCESS_CODE) {
        return null;
    }
