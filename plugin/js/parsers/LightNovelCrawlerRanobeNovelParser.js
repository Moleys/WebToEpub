...
parserFactory.register("ranobe-novels.ru", () => new LightNovelCrawlerRanobeNovelParser());


class LightNovelCrawlerRanobeNovelParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "link[rel=\"alternate\"][type=\"application/json\"][href*=\"/wp-json/wp/v2/categories/\"]";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "#js-full-content";
        if (selector) {
    ...
    extractTitleImpl(dom) {
        const selector = "h2.category-title a";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = "span[itemprop=\"creator\"]";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = "picture.category-img img";
        if (selector) {
    ...
    getInformationEpubItemChildNodes(dom) {
        const selector = "div.category-exerpt.description";
