...
parserFactory.register("justatranslatortranslations.com", () => new LightNovelCrawlerJustATranslatorTranslationsParser());


class LightNovelCrawlerJustATranslatorTranslationsParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "div.entry-content p [href*=\"justatranslatortranslations.com/\"]";
        if (!selector) {
    ...
    extractAuthor(dom) {
        const selector = "div.entry-content p";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = "div.entry-content p img";
        if (selector) {
    ...
    removeUnwantedElementsFromContentElement(element) {
        const removeSelectors = [".sharedaddy"];
        const removeTags = ["sup"];
