...
parserFactory.register("divinedaolibrary.com", () => new LightNovelCrawlerDivineDaoLibraryParser());


class LightNovelCrawlerDivineDaoLibraryParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "article .entry-content h3";
        if (!selector) {
    ...
    extractTitleImpl(dom) {
        const selector = "article .entry-content img";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = ".entry-content ul li span a";
        if (selector) {
