...
parserFactory.register("novelfull.me", () => new LightNovelCrawlerNovelFullMeParser());


class LightNovelCrawlerNovelFullMeParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "#chapter-list > li > a";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "#chapter__content .content-inner";
        if (selector) {
    ...
    extractTitleImpl(dom) {
        const selector = ".book-info .name h1";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = ".detail a[href*='/authors/'] span";
        if (selector) {
