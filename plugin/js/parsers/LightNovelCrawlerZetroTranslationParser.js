...
parserFactory.register("zetrotranslation.com", () => new LightNovelCrawlerZetroTranslationParser());


class LightNovelCrawlerZetroTranslationParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = ".wp-manga-chapter:not(.premium-block) a";
        if (!selector) {
    ...
    removeUnwantedElementsFromContentElement(element) {
        const removeSelectors = [".adsbygoogle", ".code-block"];
        const removeTags = ["h3", "script"];
