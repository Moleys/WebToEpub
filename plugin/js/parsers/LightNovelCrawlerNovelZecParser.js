...
parserFactory.register("novelzec.com", () => new LightNovelCrawlerNovelZecParser());


class LightNovelCrawlerNovelZecParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "#chap-list li a";
        if (!selector) {
    ...
    extractTitleImpl(dom) {
        const selector = ".entry-header h1";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = ".entry-header span a[href*=\"/author/\"]";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = ".entry-header img";
        if (selector) {
