...
parserFactory.register("chapmanganato.com", () => new LightNovelCrawlerReadMangaNatoParser());
parserFactory.register("readmanganato.com", () => new LightNovelCrawlerReadMangaNatoParser());


class LightNovelCrawlerReadMangaNatoParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = ".row-content-chapter li.a-h a.chapter-name";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "div.container-chapter-reader";
        if (selector) {
    ...
    extractTitleImpl(dom) {
        const selector = ".story-info-right h1";
        return selector ? dom.querySelector(selector) : super.extractTitleImpl(dom);
    }

    extractAuthor(dom) {
        const selector = "div.story-info-right td.table-value a[href*=\"author\"]";
        if (selector) {
