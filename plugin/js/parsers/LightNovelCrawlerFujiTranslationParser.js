...
parserFactory.register("fujitranslation.com", () => new LightNovelCrawlerFujiTranslationParser());


class LightNovelCrawlerFujiTranslationParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "div.entry-content p [href*=\"fujitranslation.com/\"]";
        if (!selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = "div.entry-content p strong img";
        if (selector) {
    ...
    removeUnwantedElementsFromContentElement(element) {
        const removeSelectors = ["#jp-post-flair"];
        const removeTags = [];
