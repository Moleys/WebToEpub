...
parserFactory.register("hongxiuzhao.net", () => new NovelDownloaderHongxiuzhaoParser());

class NovelDownloaderHongxiuzhaoParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        let links = [...dom.querySelectorAll(".m-chapters li > a")];
    ...
    extractTitleImpl(dom) {
        return dom.querySelector(".m-bookdetail div.f-fl > h1");
    }

    extractAuthor(dom) {
        let authorLabel = dom.querySelector(".author > a:nth-child(1)");
    ...
    extractDescription(dom) {
        let introDom = dom.querySelector(".summery");
        if (introDom != null) {
    ...
    applyContentPatch(content) {
        ...
        for (const k in hongxiuzhaoTable) {
            content.innerHTML = content.innerHTML.replaceAll(k, hongxiuzhaoTable[k]);
        }
        return content;
    }
}
