...
parserFactory.register("docln.net", () => new LightNovelCrawlerListNovelParser());
parserFactory.register("ln.hako.vn", () => new LightNovelCrawlerListNovelParser());


class LightNovelCrawlerListNovelParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = ".volume-list";
        if (!selector) {
    ...
    findContent(dom) {
        const selector = "#chapter-content p";
        if (selector) {
            let nodes = [...dom.querySelectorAll(selector)];
            if (nodes.length > 0) {
                if (nodes.length === 1) {
    ...
    findCoverImageUrl(dom) {
        const selector = ".series-cover .img-in-ratio";
        if (selector) {
