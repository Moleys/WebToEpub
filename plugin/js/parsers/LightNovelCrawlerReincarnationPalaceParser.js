...
parserFactory.register("reincarnationpalace.com", () => new LightNovelCrawlerReincarnationPalaceParser());


class LightNovelCrawlerReincarnationPalaceParser extends Parser {
    constructor() {
        super();
    }

    async getChapterUrls(dom) {
        const selector = "h4 a[href*=\"reincarnationpalace\"]";
        if (!selector) {
    ...
    extractAuthor(dom) {
        const selector = "div.elementor-widget-container p";
        if (selector) {
    ...
    findCoverImageUrl(dom) {
        const selector = "div.elementor-image img";
        if (selector) {
